...
//	Button Most;
	 ...
	 * 
	 * Der Code fuer
	 * die jeweiligen Ereignisse ist wie bei ShowClicked implementiert. Hier fuer
	 * Name aufsteigend. Fuer Meist geklickt und Name absteigend analog mit
	 * 'clicked' und 'nameDown'. Wenn das soweit funktioniert, kannst du den
...
//	}
	...
	@FXML
	private void nameUp(ActionEvent event) {
		int index = 0;
	...
	@FXML
	private void nameDown(ActionEvent event) {
		int index = 0;
	...
	@FXML
	private void mostClicked(ActionEvent event) {
		int index = 0;
		for (Song song : sort.sort(songs, "clicked", 0, songs.size() - 1)) {
			TextLiednamen.getItems().set(index++, song.getFileName());
		}
	...
	private void zeigeDatenInDerGUI(Song song) {
		...
		if (song.getCover() != "" && song.getCover() != null) {
			...
			if (cover) {
				...
				//Geht noch nicht
//				Image icon = new Image(getClass().getResource("temp/cover.jpg").toExternalForm());
//				Bild.setImage(icon);
			}
		} else {
