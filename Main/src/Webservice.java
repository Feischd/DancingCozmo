...
public class Webservice {
	...
	public Song fillSongArray(Song song) {
		...
		if (songData != "") {
			...
			song.setInformation(getInformation(songData).replaceAll("&quot;", "\"").replaceAll("&apos;", "'")
	...
	private String getSongData(Song song) {
		...
		try {
			URL url = new URL(
					" http://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=e0499e4c41404deb64230e4881e2eb27&artist="
							+ song.getArtist() + "&track=" + song.getTrack().replaceAll(" ", "%20"));
			is = url.openStream();
	...
	private int getDuration(String songData) {
		...
		try {
			duration = Integer
					.parseInt(songData.substring(songData.indexOf("<duration>") + 10, songData.indexOf("</duration>")));
		} catch (Exception e) {
		}
		return duration;
	}
	...
	private String getGenre(String songData) {
		...
		try {
			...
			while (sub.indexOf("<tag>") > -1) {
				...
				// adding <tag> to array
				genresTemp.add("0 " + sub.substring(sub.indexOf("<name>") + 6, sub.indexOf("</name>"))
						.replaceAll(" ", "").replaceAll("[A-Z]", "[a-z]"));
			}

			// Compares given Genres (tempGenre) with Genre-List; by match: Name of
			// Genre-List will be taken and 1 instead of 0 above
			...
			// checks on more than one matches by the same genre and sums the indexes above
			// the genre
			...
			// Checks on the indexes above the genres. The genre with the highest index will
			// be taken. If there are genres with the same
