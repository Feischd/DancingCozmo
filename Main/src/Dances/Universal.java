...
public class Universal extends Dance {
    public Universal(){
        laengeTanz = 23;
    }
    ...
    public void createDance(double laenge){
        ...
        try {
            ...
            pWriter.println("   robot.drive_straight(distance_mm(150), speed_mmps(150)).wait_for_completed()");
            // Turn 540 degrees to the right
            pWriter.println("   robot.turn_in_place(degrees(-540)).wait_for_completed()");
            // Drive forwards for 300 millimeters at 50 millimeters-per-second
            pWriter.println("   robot.drive_straight(distance_mm(300), speed_mmps(150)).wait_for_completed()");
            // Turn 540 degrees to the left
            pWriter.println("   robot.turn_in_place(degrees(540)).wait_for_completed()");
            // Drive forwards for 150 millimeters at 50 millimeters-per-second
            pWriter.println("   robot.drive_straight(distance_mm(150), speed_mmps(150)).wait_for_completed()");
